...
def main() -> int:
    ...
    # GL — СПЕЦИАЛЬНЫЕ БАЗОВЫЕ СТАВКИ ПРАВООХРАНИТЕЛЕЙ, разряды с 3 по 10.
    # Её тоже нет в индексе общей шкалы, и сайт её не забирал вовсе: целая
    # группа федеральной службы отсутствовала. Цена этого не абстрактная —
    # это ЕДИНСТВЕННАЯ группа, которой письмо на следующий год даёт прибавку
    # (гражданским ноль, правоохранителям 3,8 процента), то есть ровно та
    # таблица, ради которой в декабре и январе придут.
    #
    # Формат у неё тот же, что у обычных таблиц, поэтому разбирать её нечем
    # новым — не хватало только строчки здесь.
    if "GL" not in codes:
        codes.append("GL")
