...
def main() -> int:
    ...
    # Свой домен берётся из константы, а не пишется литералом: иначе
    # смена домена одной строкой роняет этот гейт на всех 165
    # страницах, объявляя собственные ссылки чужими.
    ext_pat = re.compile(
    ...
    # 17. адрес сайта только из константы. Домен ещё не выбран, и
    #     fedpayscale.com брать нельзя: на .org сидит живой конкурент на
    #     17 764 страницы. Семнадцать адресов были зашиты в четырёх модулях
    ...
    # 21. следы вычислений в ВИДИМОМ тексте. Литеральный перевод строки
    #     («обратный слэш n») прожил на живом сайте и не был пойман ни одним
    #     из 26 гейтов: все они читают РАЗМЕТКУ, а такая последовательность —
    #     валидный текст. Гейт, читающий видимый текст, нашёл вторую такую же
    #     ошибку в первую же минуту.
    #     Пять классов, у всех порог нулевой и он доказуем: сплошная проверка
    #     166 страниц после правки даёт ноль по каждому. Скрипты и стили
    #     исключены — там и слэш, и фигурные скобки законны.
    TRACES = (
        (chr(92), "обратный слэш"),
        (None, "неподставленный шаблон"),
        ("&amp;amp;", "двойное экранирование"),
        (None, "мойбейк"),
        (None, "символ нулевой ширины"),
    )
    tpl_pat = re.compile(r"\{[a-zA-Z_][\w.\[\]']*\}|%[sdf]\b")
    moji_pat = re.compile(r"\u00c3[\u0080-\u00bf]|\u00e2\u0080")
    zw_pat = re.compile(r"[\u200b\u200c\u200d\ufeff]")
    for f in htmls:
        h = f.read_text(encoding="utf-8")
        ...
        if not m:
            continue
        vis = re.sub(r"<script.*?</script>", " ", m.group(0), flags=re.S)
        vis = re.sub(r"<style.*?</style>", " ", vis, flags=re.S)
        vis = re.sub(r"<[^>]+>", " ", vis)
        hit = None
        if chr(92) in vis:
            hit = ("обратный слэш", vis.index(chr(92)))
        elif tpl_pat.search(vis):
            hit = ("неподставленный шаблон", tpl_pat.search(vis).start())
        elif "&amp;amp;" in vis:
            hit = ("двойное экранирование", vis.index("&amp;amp;"))
        elif moji_pat.search(vis):
            hit = ("мойбейк", moji_pat.search(vis).start())
        elif zw_pat.search(vis):
            hit = ("символ нулевой ширины", zw_pat.search(vis).start())
        if hit:
            what, k = hit
            problems.append(
                f"{f.relative_to(DIST)}: {what} в видимом тексте "
                f"({vis[max(0, k - 40):k + 25].strip()!r})")
            break
    ...
    #     И ГЛАВНОЕ — календарные события. Счётчик дней не поймал того, что
    #     письмо ушло в день самой проверки: сорок дней истекли бы 5 октября,
    #     а ответ года изменился 26 августа. Событие важнее срока.
    missed = payraise.missed_events()
    if missed:
        problems.append(
            "с последней проверки статуса прошли события, меняющие ответ: "
            + "; ".join(f"{d_.isoformat()} — {why}" for d_, why in missed)
            + ". Перепроверить и обновить CHECKED в payraise.py")

    # СЧЁТЧИК ДНЕЙ ПРЕДУПРЕЖДАЕТ, А НЕ ГЛУШИТ. Пропущенное событие выше —
    # блокирует и должно: случилось то, что меняет ответ. А «давно не
    # перепроверяли» — это не расхождение с миром, это напоминание.
    #
    # Цена прежнего устройства посчитана: CHECKED = 9 сентября, порог 40
    # суток, значит 20 октября сборка краснела бы, а шаг Deploy стоит НИЖЕ
    # гейтов — сайт перестал бы получать ЛЮБЫЕ изменения, включая всю
    # декабрьскую работу, и заметили бы это по тишине, а не по тревоге.
    # Остановленная выкладка — не защита от устаревшего статуса: страница на
    # проде и так осталась бы прежней, только чинить её стало бы нечем.
    stale = payraise.stale_days()
    if stale > payraise.STALE_AFTER_DAYS:
        msg = (f"статус прибавки не перепроверялся {stale} дней "
               f"(порог {payraise.STALE_AFTER_DAYS}): открыть whitehouse.gov, "
               f"Federal Register и govinfo, затем обновить CHECKED в "
               f"payraise.py")
    ...
    # 18. У СТРАНИЦЫ РОВНО ОДИН ЗАГОЛОВОК ПЕРВОГО УРОВНЯ.
    #
    #     Пятнадцать страниц разрядов — и только они — уезжали в выдачу без
    #     единого <h1>: сразу за крошками начинался <p class="sub">. На них
    #     приходится около четверти показов сайта. Ни один гейт этого не
    #     видел: разметка была валидной, описание на месте, объём в норме,
    #     ссылки целы — отсутствовала не ошибка, а ЧАСТЬ.
    #
    #     Проверяются ОБЕ стороны. Ноль — страница не называет себя ни
    #     поисковику, ни экранному диктору. Больше одного — страница
    #     называет себя дважды и разным, и тогда неизвестно, что из этого
    #     её предмет.
    #
    #     Пустая выборка — тоже отказ: гейт, которому не досталось ни одной
    #     страницы, зеленеет ровно так же, как исправный.
    seen_h1 = 0
    for f in sorted(DIST.rglob("*.html")):
        n = len(re.findall(r"<h1[\s>]", f.read_text(encoding="utf-8")))
        seen_h1 += 1
        if n != 1:
            problems.append(f"{f.relative_to(DIST)}: заголовков первого "
                            f"уровня {n}, а должен быть ровно один")
    if seen_h1 < 100:
        problems.append(f"гейт заголовков осмотрел {seen_h1} страниц — "
                        f"выборка не та, и зелёный здесь ничего не значит")
